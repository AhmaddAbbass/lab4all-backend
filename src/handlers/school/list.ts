...
export const listSchoolsHandler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    if (q && countryCode) {
      const { items, nextToken: nt } = await searchByName(countryCode, q, limit, nextToken);
      return {
        statusCode: 200,
        body: JSON.stringify({ schools: items.map(map), nextToken: nt || null }),
      };
    }

    // 4) Country + city (browse within a city)
    if (countryCode && city) {
      const { items, nextToken: nt } = await queryByCountryCity(countryCode, city, limit, nextToken);
      return {
        statusCode: 200,
        body: JSON.stringify({ schools: items.map(map), nextToken: nt || null }),
      };
