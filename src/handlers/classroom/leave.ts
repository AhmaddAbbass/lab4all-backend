...
import { deleteClassroomWithMembership } from '../../utils/database/deleteClassroomWithMembership';
 ...
 */
export const deleteMembershipHandler: APIGatewayProxyHandler = async (event) => {
  const claims = (event.requestContext.authorizer as any)?.claims;
  if (!claims) return { statusCode: 401, body: JSON.stringify({ error: 'Unauthorized' }) };

  let body: any = {};
  try { body = JSON.parse(event.body || '{}'); } catch { /* ignore, handled below */ }
  const classroomID = body.classroomID;
  if (!classroomID) return { statusCode: 400, body: JSON.stringify({ error: 'MISSING_CLASSROOM_ID' }) };

  const userId = claims.sub;

  // check membership + role
  const meToClass = await getMembershipRecord(`USER#${userId}`, `CLASSROOM#${classroomID}`);
  if (!meToClass) return { statusCode: 404, body: JSON.stringify({ error: 'MEMBERSHIP_NOT_FOUND' }) };
  ...
  try {
    if (role === 'student') {
      ...
      return { statusCode: 200, body: JSON.stringify({ message: 'Left classroom' }) };
    }

    // instructor
    const studentIds = await getStudentIDsByClassroom(classroomID); // already filters role=student
    if (studentIds.length > 0) {
      return { statusCode: 409, body: JSON.stringify({ error: 'INSTRUCTOR_CANNOT_LEAVE_WHEN_MEMBERS_PRESENT' }) };
    }

    // no students: delete instructor membership + classroom atomically
    await deleteClassroomWithMembership(userId, classroomID);
    return { statusCode: 200, body: JSON.stringify({ message: 'Classroom deleted' }) };
  } catch (err: any) {
